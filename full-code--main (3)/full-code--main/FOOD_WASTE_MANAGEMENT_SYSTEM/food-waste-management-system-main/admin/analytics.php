...
<script>
      <?php
            ...
            $female=$ro2['count'];
            $male=$row['count'];
            $q3="SELECT count(*) as count FROM food_donations where address=\"panchwati\"";
            $res3=mysqli_query($connection, $q3);
            $ro3=mysqli_fetch_assoc($res3);
            $panchwati=$ro3['count'];
            $q4="SELECT count(*) as count FROM food_donations where address=\"irvin_square\"";
            $res4=mysqli_query($connection, $q4);
            $ro4=mysqli_fetch_assoc($res4);
            $irvin_square=$ro4['count'];
            $q5="SELECT count(*) as count FROM food_donations where address=\"rajkamal\"";
            $res5=mysqli_query($connection, $q5);
            $ro5=mysqli_fetch_assoc($res5);
            $rajkamal=$ro5['count'];
            $q5="SELECT count(*) as count FROM food_donations where address=\"gadge_nagar\"";
            $res5=mysqli_query($connection, $q5);
            $ro5=mysqli_fetch_assoc($res5);
            $gadge_nagar=$ro5['count'];
            

    ?>
var xValues = ["Male","Female"];
var xplace=["panchwati","irvin_square","rajkamal","gadge_nagar"];
var yplace=[<?php echo json_encode($panchwati,JSON_HEX_TAG);?>,<?php echo json_encode($irvin_square,JSON_HEX_TAG);?>,<?php echo json_encode($rajkamal,JSON_HEX_TAG);?>,<?php echo json_encode($gadge_nagar,JSON_HEX_TAG);?>];
