...
<body>
        ...
        <br>
          ...
          <form method="post">
            ...
            <select id="location" name="location">
               <option value="panchwatii">panchwati</option>
    ...
    if(isset($_POST['location'])) {
      ...
      $sql = "SELECT * FROM cloth_donations WHERE address='$location'";
      $result=mysqli_query($connection, $sql);
    //   $result = $conn->query($sql);
      
      // If there are results, display them in a table
      if ($result->num_rows > 0) {
        // echo "<h2>cloth Donate in $location:</h2>";
        ...
        while($row = $result->fetch_assoc()) {
            echo "<tr><td data-label=\"name\">".$row['name']."</td><td data-label=\"type_of_cloth\">".$row['type_of_cloth']."</td><td data-label=\"size\">".$row['size']."</td><td data-label=\"phone\">".$row['phone']."</td><td data-label=\"quantity\">".$row['quantity']."</td><td data-label=\"Address\">".$row['address']."</td><td data-label=\"email\">".$row['email']."</td></tr>";
